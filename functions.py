...
def estimate_price():
    ...
    The user is asked to enter the address of the house for which the price prediction is required,
    so that the method can be executed on the corresponding object.
    """
...
def add_new_house():
    ...
    longitude = float(go_back(input("Enter the longitude of the house (roughly between -125 and -114): ")))
    
    if -125 > longitude or longitude > -114:
        print("Coordinates not valid. Taking you back to the menu.")
        house_wrapper()
        
    latitude = float(go_back(input("Enter the latitude of the house (roughly between 32 and 42): ")))
    
    if 32 > latitude or latitude > 42:
        print("Coordinates not valid. Taking you back to the menu.")
        house_wrapper()
...
def go_back(entry):
    """
    Allows you to always go back to the wrapper by typing BACK.
    """
